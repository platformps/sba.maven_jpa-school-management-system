...
public class SchoolManagementSystem implements Runnable {
    ...
    private Integer getCourseRegistryInput() {
        CourseService courseservice = new CourseService(MANAGEMENT_SYSTEM);
        List<String> listOfCoursesIds= new ArrayList<>();
        for (CourseInterface course:courseservice.getAllCourses()) {
            listOfCoursesIds.add(course.getId().toString());
